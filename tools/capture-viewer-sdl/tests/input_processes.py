...
with tempfile.TemporaryDirectory(prefix="js-input-", dir="/tmp") as directory:
    media, control = [str(pathlib.Path(directory) / n) for n in ("media", "input")]
    source = subprocess.Popen([sys.argv[1], media, control], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    try:
        deadline = time.monotonic() + 5
        while not (pathlib.Path(media).exists() and pathlib.Path(control).exists()):
            if source.poll() is not None or time.monotonic() >= deadline:
                raise AssertionError("source failed to listen")
            time.sleep(0.01)
        viewer = subprocess.run([sys.argv[2], "--cpu-socket", media, "--input-socket", control,
...
with tempfile.TemporaryDirectory(prefix="js-exit-", dir="/tmp") as directory:
    ...
    with report.open("w") as log:
        source = subprocess.Popen([sys.argv[1], media, control, "--report-state"], stdout=log, stderr=subprocess.PIPE, text=True)
        viewer = None
        try:
            deadline = time.monotonic() + 5
            while not (pathlib.Path(media).exists() and pathlib.Path(control).exists()):
                ...
                time.sleep(0.01)
            viewer = subprocess.Popen([sys.argv[2], "--cpu-socket", media, "--input-socket", control, "--input-self-test"],
